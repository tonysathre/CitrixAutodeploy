...
task Test {
    $Pester = Get-Command -Name 'Invoke-Pester' -ErrorAction SilentlyContinue
    if (-not $Pester) {
        Install-Module -Name 'Pester' -RequiredVersion '5.5.0' -Force -Scope CurrentUser
    }

    try {
        $PesterResult = Invoke-Pester -Path "${PSScriptRoot}\tests" -CI
